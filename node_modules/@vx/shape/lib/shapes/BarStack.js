...
function BarStack(_ref) {
  ...
  var barWidth = (0, _objHasMethod.default)(xScale, 'bandwidth') ? xScale.bandwidth() : Math.abs(xRange[xRange.length - 1] - xRange[0]) / xDomain.length;
  var barStacks = stacks.map(function (barStack, i) {
    ...
    return {
      ...
      bars: barStack.map(function (bar, j) {
        ...
        var barX = (0, _objHasMethod.default)(xScale, 'bandwidth') ? xScale(x(bar.data)) : Math.max(xScale(x(bar.data)) - barWidth / 2);
        return {
          bar: bar,
          key: key,
          index: j,
